...
class VariableConverter(ABC):
    ...
    def era5_to_om(
        self, data: Union[float, np.ndarray, torch.Tensor]
...
class WindConverter(VariableConverter, ABC):
    ...
    def om_to_era5(
        self, data: Union[np.ndarray, torch.Tensor], trigeometry: Callable
...
REGISTRY = {
    converter.data_var: converter
    for converter in [
        TemperatureConverter(
            "2m_temperature", om_name="temperature_2m", short_code="t2m", unit="K"
        ),
        PrecipitationConverter(
            "total_precipitation", om_name="precipitation", short_code="tp", unit="m/h"
        ),
        EastWindConverter(
        ...
        SurfacePressureConverter(
            "surface_pressure", om_name="surface_pressure", short_code="sp", unit="Pa"
        ),
    ]
}
